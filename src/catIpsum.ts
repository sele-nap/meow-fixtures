...
export function randomParagraph(min = 3, max = 6): string {
  const count = min + Math.floor(Math.random() * (max - min + 1))
  return shuffle(SENTENCES).slice(0, count).join(' ')
}

export function randomText(paragraphCount = 4): string {
  return Array.from({ length: paragraphCount }, () => randomParagraph()).join('\n\n')
}
