...
import { randomName, randomText, Locale } from './catIpsum'
...
export const ALL_FORMATS = ['png', 'jpeg', 'json', 'csv', 'txt', 'pdf', 'sql', 'md', 'types'] as const
export type Format = typeof ALL_FORMATS[number]

// ── Data building ────────────────────────────────────────────────────────────

async function buildCats(options: GenerateOptions, rng: RNG): Promise<CatFixture[]> {
...
export async function generate(options: GenerateOptions): Promise<void> {
  ...
  for (const cat of cats) {
    if (formats.includes('png')) {
      fs.writeFileSync(path.join(imagesDir, catFilename(cat, 'png', prefix)), cat.pngBuffer)
    }
    if (formats.includes('jpeg')) {
      fs.writeFileSync(path.join(imagesDir, catFilename(cat, 'jpeg', prefix)), cat.jpegBuffer)
    }
  }
...
function writeSql(cats: CatFixture[], output: string, prefix?: string): void {
  ...
  const lines = [
    ...
    cats.map((cat, i) => {
      ...
      return `  ('${esc(cat.id)}', '${esc(cat.name)}', '${esc(cat.text)}', ` +
...
function writeMd(cats: CatFixture[], output: string, prefix?: string): void {
  const header = ['# Cat Fixtures', '', `_${cats.length} cats generated_`, '', '---', '']
  const tableHeader = ['| # | Name | ID | Preview |', '|---|------|----|---------|']
  const rows = cats.map(cat => {
  ...
  const content = [...header, ...tableHeader, ...rows, ...details, ...catDetails]
...
async function writePdf(cats: CatFixture[], output: string, _prefix?: string): Promise<void> {
